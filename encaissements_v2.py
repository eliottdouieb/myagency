...
def check_oublie_credit_ou_debit(df):
    a=df[df['Account Global']==411000]
    b=df[df['Account Global']!=411000]
...
def check_lignes_comptables(df):
    ...
    for i in invoice:
        ...
        if check_oublie_credit_ou_debit(df_provisoire):
            ...
            df.loc[df_provisoire[df_provisoire['Account Global']==411000].index, "Credit"] = df_provisoire[df_provisoire['Account Global']!=411000]['Debit'].sum()
            df_provisoire.loc[df_provisoire[df_provisoire['Account Global']==411000].index, "Debit"] = df_provisoire[df_provisoire['Account Global']!=411000]['Credit'].sum()
            df_provisoire.loc[df_provisoire[df_provisoire['Account Global']==411000].index, "Credit"] = df_provisoire[df_provisoire['Account Global']!=411000]['Debit'].sum()
            log_piece.append(f"🔧 encaissement {i}- Credit ou Debit omis")
        ...
        if round(df_provisoire["Debit"].sum(), 2) != round(df_provisoire["Credit"].sum(), 2):
            ...
            if check_mauvais_emplacement_credit(df_provisoire)==True:
                ...
                index_cond = df_provisoire[df_provisoire['Account Global']!=411000].index[df_provisoire[df_provisoire['Account Global']!=411000]['Credit'] != 0]
            ...
            if check_mauvais_emplacement_debit(df_provisoire)==True:
                ...
                index_cond = df_provisoire[df_provisoire['Account Global']!=411000].index[df_provisoire[df_provisoire['Account Global']!=411000]['Debit'] != 0]
        ...
        if log_ko==False:
             log_generale.append(f"✅ encaissement {i} : OK , {log_piece}")
        if log_ko==True:
            log_generale.append(f"❌ encaissement {i} : KO , {log_piece}")

    return log_generale,Compte_Tiers_invalide,encaissements_ko


def run_interface():
    st.title("📊 Contrôle automatique des écritures des encaissements")
    ...
    if uploaded:
        ...
        if Compte_Tiers_invalide > 0:
            st.warning(
                "Des encaissements KO subsistent (Account Client invalide). "
                "Modifie le tableau puis clique sur « Valider les corrections »."
            )

            df_ko = df[(df["Invoice #"].isin(encaissements_ko)) & (df["Account Global"] == 411000)].copy()
            ...
            edited = st.data_editor(
                df_unique[["Invoice #", "Account Client", "Debit", "Credit", "Name", "Concierge",'Payment Date']],
                key=editor_key,
                hide_index=True,
            )
                ...
                for _, r in edited.iterrows():
                    if r['Account Client'] != "411-NO MEMBER ACCOUNT":
                        ...
                        with st.spinner("Mise à jour des comptes tiers dans le CRM (seulement les lignes modifiées)…"):
                            invoice_number = str(r["Invoice #"]).strip()
                            compte_value = str(r["Account Client"]).strip()
                            date = _to_iso_date(str(r['Payment Date']).strip())
